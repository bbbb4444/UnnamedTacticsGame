...
*/
    ...
    {
        if (TileSelector.SelectableTiles.Contains(tile))
        {
    ...
    public void Move()
    {
        if (_path.Count > 0)
        {
            ...
            {
                bool jump = (Math.Abs(transform.position.y - target.y) > 0.001f);
                 if (jump)
                {
                    Jump(target);
                }
...
*/
    ...
    protected void FindPath(Tile target)
    {
        ...
        currentTile.h = Vector3.SqrMagnitude(currentTile.transform.position - target.transform.position);
    ...
    protected virtual void OnStopMoving()
    {
        ...
        Moving = false;
        TileSelector.ResetSelectableTiles();
